                            ...
                            const int height)
{
  ...
  /* Lens defocus is sampled per ray, so the blur is stochastic: every sample looks through a
   * different point of the aperture. DLSS-RR has no model for that -- its guides describe one
   * surface per pixel -- so out-of-focus areas come back speckled. This only bites in camera view,
